...
- The language list is now a simple row per language: its flag, then its name written the way that language writes it. The rollout groupings and the status line under every entry are gone; a language you cannot pick yet is simply greyed out.
- German and French are now fully translated and selectable in Settings. Their complete catalogs and responsive layouts passed the release audits, so players can switch immediately among English, Italian, German, and French without fallback text filling untranslated gaps.
...
- Hero selection now has a sort control, in the same compact style the Workshop uses: order the available heroes by level, Echo Resonance, class or name, each with an up and a down arrow. It appears in every hero picker - expeditions, raids, the Tower and Echo Descent - and applies the moment you choose it, without losing the team you have assembled. Level, highest first, is still the default.
- Companion selection gained the same control, ordering your companions by level, rarity or family in either direction. Family follows the order you met them in, from the earliest dungeon to the latest, so every companion of a family sits together.
